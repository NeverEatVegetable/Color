    ...
    execute: function () {
      ...
      _export("HallServer", HallServer = class HallServer {
        constructor(getRedisClient, hallServerCfg, getHallServerCfg) {
          ...
          this.server.flows.preApiCallFlow.push( /*#__PURE__*/_asyncToGenerator(function* (v) {
            var conf = v.service.conf;
            ...
            if (!conf.skipPlayerAuth) {
              ...
              var vRet = yield (_crd && PlayerAuthHelper === void 0 ? (_reportPossibleCrUseOfPlayerAuthHelper({
                error: Error()
            ...
            var apiCryptoImpl = _this.apiCryptoImpls[conf.cryptoMode];

            if (apiCryptoImpl) {
              var ret = yield apiCryptoImpl.decryptionReq(v.req);
        ...
        start() {
          ...
          return _asyncToGenerator(function* () {
            yield _this2.matchRequestTerminal.start();
            yield _this2.server.autoImplementApi(path.resolve(__dirname, 'api'));
            yield _this2.server.start();
            var joinErr = yield _this2.gameClusterTerminal.joinCluster();
            ...
            if (joinErr) {
              ...
              (_ref2 = _crd && logger === void 0 ? (_reportPossibleCrUseOflogger({
                error: Error()
              }), logger) : logger) == null || _ref2.error("HallServer: 游戏集群终端连接失败:" + joinErr + '. 即将停止服务!');
              yield _this2.stop();
              return;
            } else {
              var _ref3;
              ...
              (_ref3 = _crd && logger === void 0 ? (_reportPossibleCrUseOflogger({
                error: Error()
              }), logger) : logger) == null || _ref3.log("HallServer: 游戏集群终端连接成功!");
            }
          })();
        }
        /**停止服务*/
        ...
        stop() {
          ...
          return _asyncToGenerator(function* () {
            yield _this3.gameClusterTerminal.disconnectCluster();
            yield _this3.server.stop();
            yield _this3.matchRequestTerminal.stop();
          })();
        }
