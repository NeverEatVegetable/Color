      ...
      _export("GameServerClusterMgr", GameServerClusterMgr = class GameServerClusterMgr extends (_crd && ClusterMgr === void 0 ? (_reportPossibleCrUseOfClusterMgr({
        ...
        start() {
          ...
          return _asyncToGenerator(function* () {
            //额外注册api目录下的接口
            yield _this.autoImplementApi(_crd && gameClusterServiceProto === void 0 ? (_reportPossibleCrUseOfgameClusterServiceProto({
              error: Error()
            }), gameClusterServiceProto) : gameClusterServiceProto, path.resolve(__dirname, 'api'));
            yield _superprop_getStart().call(_this);
          })();
        }
        ...
        stop() {
          ...
          return _asyncToGenerator(function* () {
            var _this2$gameServerTask;

            yield (_this2$gameServerTask = _this2.gameServerTaskCancel) == null ? void 0 : _this2$gameServerTask.cancel();
            yield _superprop_getStop().call(_this2);
          })();
        }
        /**
         * 通知游戏服务器解散房间
        ...
        notifyGameServerDismissRoom(roomRegInfo) {
          var _this3 = this;

          return _asyncToGenerator(function* () {
            var ret = yield _this3.assignTask(roomRegInfo.gameServerNodeId, 'DismissRoom', roomRegInfo.roomId);
        ...
        allotGameServerByInfos(allServerNodeList, getServerRooms, createRoomPara) {
          return _asyncToGenerator(function* () {
            var canAllotNodes = []; //按在线客户端升序
            ...
            var _loop = function* _loop() {
              ...
              if ((_s$info$allotRules = s.info.allotRules) != null && (_s$info$allotRules = _s$info$allotRules.limitRoomCountRules) != null && _s$info$allotRules.length) {
                ...
                var rules = s.info.allotRules.limitRoomCountRules.filter(r => {
                  // 有配置且不符合的过滤
                ...
                var minLimit = (_crd && arrWinner === void 0 ? (_reportPossibleCrUseOfarrWinner({
                  error: Error()
                }), arrWinner) : arrWinner)(rules, (winner, item) => winner.limitRoomCount < item.limitRoomCount ? winner : item); // 检查现有房间符合规则的数量

                var rooms = yield getServerRooms(s.info.serverNodeId);
                if (!rooms) return 0; // continue

                var currRoomCount = rooms.count(room => {
            ...
            for (var s of allServerNodeList) {
              _ret = yield* _loop();
              if (_ret === 0) continue;
            }
            ...
            var allotGameServer = (_crd && arrWinner === void 0 ? (_reportPossibleCrUseOfarrWinner({
              error: Error()
        ...
        buildRoomOnlineInfo(roomRegInfo) {
          var gameServerInfo = this.getNodeById(roomRegInfo.gameServerNodeId);
          var roomOnlineInfo = (_crd && GameServerHelper === void 0 ? (_reportPossibleCrUseOfGameServerHelper({
            error: Error()
