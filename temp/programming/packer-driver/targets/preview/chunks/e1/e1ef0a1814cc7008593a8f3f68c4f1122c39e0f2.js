  ...
  return {
    ...
    execute: function () {
      ...
      _export("FrameSyncExecutor", FrameSyncExecutor = class FrameSyncExecutor {
        ...
        constructor(onSyncOneFrame, syncFrameRate) {
        ...
        runFrameAndNext() {
          ...
          var nextFrameRunTime = this._startFrameTime + this._nextSyncFrameIndex * this._syncFrameIntervalMs; //再计算出当前时间再过多久到

          var waitMs = nextFrameRunTime - now; //当然, 也可能是负数(即在这现在之前就应该跑到了, 但都超过时间了, 则置为0, 尽最大努力执行)
        ...
        _syncOneFrameHandler() {
          ...
          var currFrameArrIndex = this._prevSyncFrameIndexArrIndex + 1; //最大帧索引推进
        ...
        addPlayerInpFrame(playerId, inpFrameType, setOthersProp) {
          ...
          var nextFrameArrIndex = this._prevSyncFrameIndexArrIndex + 1; //更新最大帧索引
        ...
        buildAfterFrames(startFrameIndex) {
          ...
          var aFrames = this.requestFrames(startFrameIndex);
          var msg = {
        ...
        requestFrames(beginFrameIndex, endFrameIndex) {
          ...
          for (var i = beginFrameIndex; i <= endFrameIndex; i++) {
            var arrIndex = this._allFrameIndexMapArrIndex[i];
            var frame = typeof arrIndex !== 'undefined' ? this._allFrames[arrIndex] : null;
