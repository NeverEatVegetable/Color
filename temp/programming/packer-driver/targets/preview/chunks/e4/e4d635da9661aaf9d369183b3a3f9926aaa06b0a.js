  ...
  return {
    ...
    execute: function () {
      ...
      _export("GameConnMgr", GameConnMgr = class GameConnMgr {
        ...
        disconnectClearData(connId, player, triggerEvent) {
        ...
        startWaitAuth(conn) {
          var hd = this.waitAuthConnectionsTimeHD.get(conn.connectionId);
        ...
        clearWaitAuth(connId) {
          var hd = this.waitAuthConnectionsTimeHD.get(connId);
          if (hd) clearTimeout(hd);
        ...
        startWaitReconnect(connId, player) {
          var hd = this.waitConnectionTimeHD.get(player.authInfo.playerId);
          if (hd) clearTimeout(hd);
        ...
        clearWaitReconnect(playerId) {
          var hd = this.waitConnectionTimeHD.get(playerId);
          if (hd) clearTimeout(hd);
        ...
        tryReconnect(playerId) {
          var waitPlayer = this.waitConnectionPlayer.get(playerId);
        ...
        connReconnect(conn, playerToken, roomWaitReconnectTime) {
          var _this = this;

          return _asyncToGenerator(function* () {
            //先验证token
            var authInfoRet = yield (_crd && PlayerAuthHelper === void 0 ? (_reportPossibleCrUseOfPlayerAuthHelper({
              error: Error()
            ...
            var authInfo = authInfoRet.data; //通过以及拿到playerId,尝试断线重连数据获取

            var player = _this.tryReconnect(authInfo.playerId);
            ...
            var err = _this.checkConnAuthorizing(conn, player);
            ...
            _this.setConnAuthorized(conn, player);

            _this.triggerConnReconnect(conn, player);
        ...
        connAuthorize(conn, playerToken, authPlayerPara, roomWaitReconnectTime) {
          var _this2 = this;
          ...
          return _asyncToGenerator(function* () {
            ...
            var authInfo = authInfoRet.data;
            if (roomWaitReconnectTime === undefined) roomWaitReconnectTime = _this2.defaultWaitReconnectTime;
            var playerInfo = {
            ...
            var err = _this2.checkConnAuthorizing(conn, player);
            ...
            _this2.setConnAuthorized(conn, player);
        ...
        getPlayersConn(playerIds) {
          ...
          for (var playerId of playerIds) {
            var _conn = this.currConnections.get(playerId);

            if (_conn) connList.push(_conn);
          }
        ...
        getPlayersConnFromPlayerInfos(playerInfos) {
          ...
          for (var p of playerInfos) {
            var _conn2 = this.currConnections.get(p.playerId);

            if (_conn2) connList.push(_conn2);
          }
        ...
        triggerConnAuthing(conn, player) {
          for (var i = 0; i < this.connAuthingHandlers.length; i++) {
            try {
              var err = this.connAuthingHandlers[i](conn, player);
              if (err) return err;
            } catch (e) {
        ...
        triggerAuthedPlayerDisconnect(connId, player) {
          ...
          setTimeout((connId, player) => {
            if (connId && player) {
              for (var i = 0; i < this.authedPlayerDisconnectHandlers.length; i++) {
                try {
        ...
        triggerConnDisconnect(connId, player) {
          for (var i = 0; i < this.connDisconnectHandlers.length; i++) {
            try {
              var wait = this.connDisconnectHandlers[i](connId, player);
        ...
        triggerConnReconnect(conn, player) {
          for (var i = 0; i < this.connReconnectHandlers.length; i++) {
            try {
