      ...
      */
      _export("MatcherFixedTeams", MatcherFixedTeams = class MatcherFixedTeams {
        ...
        matchAllReqs(allReqs) {
          ...
          var _loop = function _loop(checkReq) {
            ...
            if (matchReqs.length > 0) {
              var _roomTeamPCount = fromInfo.teamsPlayerIds.map(t => t.playerIds.length);

              var _ret = _this.matchPlayerReqs(playerReqs, checkReq, _roomTeamPCount, false, matchReqs);

              if (_ret.matchRequestPlayerResults && _ret.matchRequestPlayerResults.length > 0) {
                var _roomJoinUsMatch = true;

                var _matchPlayerCount = (_crd && arrSum === void 0 ? (_reportPossibleCrUseOfarrSum({
                  error: Error()
                }), arrSum) : arrSum)(_ret.matchRequestPlayerResults, r => r.matchPlayerResults.length);

                if (fromInfo.currPlayerCount + _matchPlayerCount >= checkReq.maxPlayers) {
                  //如果匹配完玩家满员,则关闭招人匹配
                  _roomJoinUsMatch = false;
                } //匹配成功!


                joinRoomResults.push({
                  joinRoomId: fromInfo.roomId,
                  matchRequestPlayerResults: _ret.matchRequestPlayerResults,
                  roomJoinUsMatch: _roomJoinUsMatch
                });
              }
            }
          };
          ...
          var groupReqs = (_crd && arrGroup === void 0 ? (_reportPossibleCrUseOfarrGroup({
            error: Error()
          }), arrGroup) : arrGroup)(playerReqs, r => r.matchReqGroupKey);

          for (var group of groupReqs) {
            var reqs = group[1];
            if (reqs.length <= 0) continue; //因为同分组下,固定队伍的配置是一致的,只要拿第一个就能获取要求的最小玩家数

            var params = reqs[0].matcherParams;
            var pTeams = params.fixedTeamList;
            var minPCount = (_crd && arrSum === void 0 ? (_reportPossibleCrUseOfarrSum({
              error: Error()
            }), arrSum) : arrSum)(pTeams, t => t.minPlayers); //未被匹配列表

            var waitReqs = reqs; //整个分组只要剩余匹配数满足最少玩家数,则会继续匹配
            ...
            while (waitReqs.length > 0 && (_crd && arrSum === void 0 ? (_reportPossibleCrUseOfarrSum({
              ...
              var _checkReq = waitReqs[0];
              var roomTeamPCount = pTeams.map(t => 0);
              var ret = this.matchPlayerReqs(waitReqs, waitReqs[0], roomTeamPCount, false, waitReqs.slice());
              ...
              if (matchPlayerCount < _checkReq.maxPlayers) {
              ...
              createRoomResults.push({
                createRoomPara: {
                  roomName: '系统匹配的房间',
                  ownerPlayerId: '',
                  maxPlayers: _checkReq.maxPlayers,
                  isPrivate: false,
        ...
        matchPlayerReqs(allReqs, checkReq, checkReqTeamPlayers, checkReqSelfApply, matchPlayerReqs) {
          ...
          var matchRequestPlayerResults = []; //匹配器参数(匹配器收到时已经初始化过,确保有不为空)

          var params = checkReq.matcherParams;
          var remainPCount = checkReqTeamPlayers.slice();
          ...
          var hasRemain = false; //优先筛选出刚好符合各队伍的剩余人数

          var waitFullMatchReqs = matchPlayerReqs.slice();
          var fullMatchResult = [];

          var _loop2 = function _loop2(i) {
            if (remainPCount[i] <= 0) return 0; // continue
            //如果当前队伍直接满员,则忽略
            //找到玩家数量和剩下数量一致的匹配请求

            var matchReq = waitFullMatchReqs.find(r => r.matchFromInfo.playerIds.length === remainPCount[i]);
          ...
          for (var i = 0; i < remainPCount.length; i++) {
            _ret2 = _loop2(i);
            if (_ret2 === 0) continue;
          }
          ...
          if (hasRemain) {
            ...
            var fullTeamCounts = params.fixedTeamList.map(t => {
              return {
                min: t.maxPlayers,
                max: t.maxPlayers
              };
          ...
          if (hasRemain) {
            ...
            var minTeamCounts = params.fixedTeamList.map(t => {
              return {
                min: t.minPlayers,
                max: t.maxPlayers
              };
            ...
            if (!hasRemain) {
              var _loop3 = function _loop3(teamIndex) {
                if (remainPCount[teamIndex] <= 0) return 1; // continue
                //如果当前队伍直接满员,则忽略
                //遍历剩余匹配,推满本队玩家

                for (var _i = 0; remainPCount[teamIndex] > 0 && _i < matchPlayerReqs.length; _i++) {
                  var req = matchPlayerReqs[_i];
                  var pCount = req.matchFromInfo.playerIds.length;
                  ...
                  matchPlayerReqs.splice(_i, 1); //删掉匹配上的
              ...
              //满足最低要求了,则开始尽量推满!
              for (var teamIndex = 0; matchPlayerReqs.length > 0 && teamIndex < remainPCount.length; teamIndex++) {
                if (_loop3(teamIndex)) continue;
              }
            }
          }
        ...
        extractTeamCombination(params, remainPCount, remainRange, matchPlayerReqs, matchRequestPlayerResults) {
          ...
          var waitReqs = matchPlayerReqs.slice(); //结果对象临时放,最终成功才追加到传入的结果数组

          var tmpResults = []; //是否还有空的

          var hasRemain = false;
          var tmpRemainPCount = remainPCount.slice();

          var _loop4 = function _loop4(teamIndex) {
            var min = remainRange[teamIndex].min;
            var max = remainRange[teamIndex].max;
            var maxRemain = max - min;
            if (tmpRemainPCount[teamIndex] <= maxRemain) return 0; // continue
            //如果当前队伍直接满员,则忽略
            ...
            var startRemain = tmpRemainPCount[teamIndex];
            var reqs = waitReqs.slice(); //匹配请求列表复制一份,方便尝试完没发现合适的组合就跳过
            ...
            */
            ...
            do {
              ...
              for (var c = 0, count = tmpRemainPCount[teamIndex]; c < count; c++) {
                var hasCon = false; //开始遍历除了startI位置的其他匹配

                for (var i = 0; i < reqs.length && tmpRemainPCount[teamIndex] > maxRemain; i++) {
                  var req = reqs[i];
          ...
          for (var teamIndex = 0; teamIndex < tmpRemainPCount.length; teamIndex++) {
            _ret3 = _loop4(teamIndex);
            if (_ret3 === 0) continue;
            if (_ret3 === 1) break;
          }
        ...
        procNewMatchReq(newMatchReq) {
          //新匹配进来先检查参数和初始化参数!
          var params = newMatchReq.matcherParams;
          var teams = newMatchReq.teamParams;
          ...
          if (teams.fixedTeamInfoList && teams.fixedTeamInfoList.length) {
            ...
            for (var i = 0; i < teams.fixedTeamCount; i++) {
        ...
        onNewMatchReq(newMatchReq, allReqs) {
          var procRet = this.procNewMatchReq(newMatchReq);
